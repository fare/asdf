...
(defpackage :sb-texinfo
  ...
  (:export #:generate-includes #:document-package #:complete-texinfo-template #:*not-symbols*)
  (:documentation
...
(defun render-package-name (package)
  (nstring-downcase
  ...
  (let* ((kind (get-kind doc))
         (name (nstring-downcase
                (if (eq 'package kind)
                    ...
                    (format nil "~A-~A-~A"
                            (render-package-name (get-package doc))
                            (case (get-kind doc)
      ...
      (do ((begin nil)
          ...
          ((= i (length line))
           ...
           (when (and begin (or (> i (1+ begin))
             ...
             (grab begin i))
           (nreverse result))
        ;; (format t "begin: ~A, maybe-begin: ~A, i: ~A~%" begin maybe-begin i)
        (cond
...
be outputted by `generate-includes' beforehand."
  (let* ((template-pathname (merge-pathnames template-pathname))
         ...
         (prefix-length (length *prefix-of-lines-to-be-replaced*))
         (generated-files (directory (merge-pathnames "*.texinfo" include-directory))))
    (with-open-file (in template-pathname)
      (with-open-file (out dest-pathname
                           :direction :output
                           :if-exists :supersede)
        (loop for line = (read-line in nil nil)
              while line
              if (uiop:string-prefix-p *prefix-of-lines-to-be-replaced* line)
                do (let* ((package-name (subseq line prefix-length))
                          (package (or (find-package package-name)
                                       (error "No package named ~A." package-name)))
                          (filename-prefix (render-package-name package))
                          (files
                            (loop for path in generated-files
                                  when (uiop:string-prefix-p filename-prefix
                                                             (file-namestring path))
                                    collect path)))
                     (dolist (file files)
                       (format out "@include ~A~%" file)))
              else
